    ...
    queue.enqueue(async () => {
        ...
        switch (interaction.customId) {
            case "selectroles_announcement": {
                ...
                await handleSelectableRole(role, member, interaction);
                return;
            }
            ...
            default: {
                await interaction.editReply({ content: "실패했어요.. 관리자에게 문의해주세요!" });
                return;
            }
        }

        await interaction.editReply({ content: "<a:check:1061576200075620362> 설정되었어요!" });
    });
};
...
const handleSelectableRole = async (role: Role, member: GuildMember, interaction: MessageComponentInteraction) => {
    if (member.roles.cache.has(role.id)) {
        ...
        await interaction.editReply({ content: "➖ 해당 역할을 제거했어요!" });
    } else {
        await member.roles.add(role);
        await interaction.editReply({ content: "➕ 해당 역할을 추가했어요!" });
    }
};
